...
//           	UpdateHistogramReflect(thePostPoint,theTrack);
      ...
      return;
         ...
         if (thePostPVname=="PhotonDetX")  fEventAction->AddPhotCountX(1);
         if (thePostPVname=="PhotonDetY")  fEventAction->AddPhotCountY(1);
         if (thePostPVname=="PhotonDetZ"){
	   fEventAction->AddPhotCountZ(1);//add
	   fEventAction->AddPhottime(theTrack->GetGlobalTime());//add
	   fEventAction->AddPhotlasttime(theTrack->GetGlobalTime());//add
	   G4cout << "The time when a photn reached MPCC (for z-read out fiber) = " << theTrack->GetGlobalTime() << G4endl;
	 }
         if (thePostPVname=="PhotonDetX"||thePostPVname=="PhotonDetY"||thePostPVname=="PhotonDetZ") {
            ResetCounters();
            theTrack->SetTrackStatus(fStopAndKill);
				return;
         }
